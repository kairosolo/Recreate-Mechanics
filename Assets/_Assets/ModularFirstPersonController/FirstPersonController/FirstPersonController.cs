...
public class FirstPersonController : MonoBehaviour
{
    ...
    #endregion Camera Zoom Variables

    #endregion Camera Movement Variables
    ...
    private void Update()
    {
        ...
        if (handTransform != null)
        {
            ...
            Vector3 targetPosition = playerCamera.transform.position +
                                  (playerCamera.transform.right * 0f) +
            ...
            handTransform.position = Vector3.Lerp(handTransform.position, targetPosition, Time.deltaTime * 10f);

            // Smoothly rotate hand to target rotation
            handTransform.rotation = Quaternion.Slerp(handTransform.rotation, targetRotation, Time.deltaTime * 10f);
        }
        ...
        if (isHolding)
        {
            ...
            if (Input.GetKey(KeyCode.F) && isCharging)
            {
                ...
                ActionScript.Instance.ActionTextState(true);
                ActionScript.Instance.UpdateChargeText(chargePercent);
            }
            else if (!isCharging)
            {
                ActionScript.Instance.ThrowText();
            }
        }
    ...
    private void DetectInteractables()
    {
        ...
        if (Physics.Raycast(origin, direction, out RaycastHit hit, interactableDistance))
        {
            Debug.DrawRay(origin, direction * interactableDistance, Color.green);
            if (hit.collider.TryGetComponent<Interactable>(out Interactable interactable))
            {
                ...
                {
                    ActionScript.Instance.ActionTextState(true);
                    ActionScript.Instance.PickUpText();
                }
            }
        }
        else if (!isCharging)
        {
            ActionScript.Instance.ActionTextState(false);
        }
    }
    ...
    private float GetChargePercent()
    {
        ...
        return Mathf.Clamp01(holdTime / maxChargeTime);
    }
    ...
    private void ThrowObject()
    {
        ...
        Transform heldObject = handTransform.GetChild(0);
        if (heldObject != null && heldObject.TryGetComponent<Interactable>(out Interactable interactable))
        {
            Vector3 throwDirection = playerCamera.transform.forward;
            interactable.Throw(throwForce, throwDirection);
            isHolding = false;
        }

        // Reset throwing states
        isCharging = false;
        ActionScript.Instance.ActionTextState(false);
    }
...
public class FirstPersonControllerEditor : Editor
{
    ...
    private SerializedObject SerFPC;
    ...
    public override void OnInspectorGUI()
    {
        ...
        SerializedProperty handTransform = SerFPC.FindProperty("handTransform");
        SerializedProperty interactableDistance = SerFPC.FindProperty("interactableDistance");
        SerializedProperty isHolding = SerFPC.FindProperty("isHolding");

        EditorGUILayout.PropertyField(handTransform, new GUIContent("Hand Transform", "Transform that represents where held objects will be placed."));
        EditorGUILayout.Slider(interactableDistance, 0.5f, 100f, new GUIContent("Interaction Distance", "How far the player can reach to interact with objects."));

        GUI.enabled = false; // Make the isHolding field read-only
        EditorGUILayout.PropertyField(isHolding, new GUIContent("Is Holding", "Shows if the player is currently holding an object."));
        GUI.enabled = true;

        #endregion Interaction Setup
